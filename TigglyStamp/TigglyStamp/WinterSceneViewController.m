...
-(void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    DebugLog(@"");
//    [multiTouchForTouchView addObject:touches];
...
-(void) touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
   DebugLog(@"");
//    if([multiTouchForTouchView count]>1){
//        [multiTouchForTouchView addObject:touches];
...
-(IBAction)onButtonClicked:(id)sender{
    ...
    if ([btn tag] == TAG_RIGHT_TICK_BTN) {
        if (![btn isHidden]) {
            ...
            if ([[[NSUserDefaults standardUserDefaults] valueForKey:SAVE_ART] isEqualToString:@"yes"]) {
                [cameraButton setHidden:NO];
                [videoButton setHidden:NO];
            }else if ([[[NSUserDefaults standardUserDefaults] valueForKey:SAVE_ART] isEqualToString:@"no"]) {
    ...
    if ([btn tag] == TAG_CURL_CONFIRMED_BTN) {
        ...
        self.curlView.cylinderPosition = CGPointMake(self.mainView.bounds.size.width, self.mainView.bounds.size.height);
        [self.curlView curlView:self.mainView cylinderPosition:CGPointMake(-400 ,-400) cylinderAngle:3.5*M_PI_4 cylinderRadius:UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad? 160: 70 animatedWithDuration:1.2];
        
         removeCurlTimer = [NSTimer scheduledTimerWithTimeInterval:1.3 target:self selector:@selector(removeCurl) userInfo:nil repeats:NO];
         */
    }
}
